...
class HomeCubit extends Cubit<HomeScreenState> {
  ...
  addNewItemToCartFromHomeScreen({itemId, orderCount}) {
    var newList =
        popularFoodList.firstWhere((element) => element.itemId == itemId);
    var model = ItemModel(
        orderCount: orderCount,
        oldPrice: newList.oldPrice,
        itemTitle: newList.itemTitle,
  ...
  addNewItemToCartFromFeedsScreen({itemId, orderCount}) {
    var newList =
        listFeedsSearch.firstWhere((element) => element.itemId == itemId);
    var model = ItemModel(
        orderCount: orderCount,
        oldPrice: newList.oldPrice,
        itemTitle: newList.itemTitle,
  ...
  getCategory() async {
    FirebaseFirestore.instance
        ...
        .listen((event) {
      listCategory =
          event.docs.map((x) => Category.fromJson(x.data())).toList();
  ...
  getItems() async {
    FirebaseFirestore.instance.collection('Items').snapshots().listen((event) {
      ...
      popularFoodList =
          listItems.where((element) => element.isPopular).toList();
      emit(SelectCategoryState());
    });
  }
  ...
  changeItemFavouriteState({bool isFavourite = false, int itemId}) {
    FavouritModel model = FavouritModel(
        isFavourit: !isFavourite, ItemId: itemId, UesrMobile: Global.mobile);
      ...
      navigateTo(
          ...
          subCategoryScreen(
            categoryTitle: listCategory
                .firstWhere(
                    (element) => element.categoryId == selectedCategoryId)
                .categoryTitle,
          ));
    } else {
  ...
  selectSubCategory({supCategoryId, context}) async {
    ...
    listItemsSearch = listItems
        .where((element) => element.supCategoryId == supCategoryId)
        .toList();
    emit(SelectCategoryState());
      ...
      navigateTo(
          ...
          ItemsScreen(
              subcategoryTitle: listItems
                      .firstWhere(
                          (element) => element.supCategoryId == supCategoryId)
                      .supCategoryTitle ??
                  ''));
    } else {
      EasyLoading.showError('لا يوجد بيانات');
  ...
  searchInItems(String value) {
    if (value.trim() != '') {
      ...
      listItemsSearch = listItems
          .where((element) => element.supCategoryId == selectedSubCategoryId)
          .toList();
    }
    emit(SearchSubCategoryState());
  }
